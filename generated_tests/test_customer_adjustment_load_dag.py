...
DAG_FILE = Path(__file__).resolve().parent.parent / "generated_dags" / "customer_adjustment_load_dag.py"


def load_dag_module():
    spec = importlib.util.spec_from_file_location("customer_adjustment_load_dag", DAG_FILE)
...
def test_dag_id_is_correct():
    module = load_dag_module()
    assert module.dag.dag_id == "customer_adjustment_load"
